...
def test_direct_equivariance(dtype,num_tol,rot_tol,direct_checkpoint,fake_puma_dataset):
    direct_inference_checkpoint_pt, _ = direct_checkpoint
    equivariance_on_pt(dtype,num_tol,rot_tol,direct_inference_checkpoint_pt,fake_puma_dataset)
...
def test_direct_mole_equivariance(dtype,num_tol,rot_tol,direct_mole_checkpoint,fake_puma_dataset):
    direct_mole_inference_checkpoint_pt, _ = direct_mole_checkpoint
    equivariance_on_pt(dtype,num_tol,rot_tol,direct_mole_inference_checkpoint_pt,fake_puma_dataset)
...
def test_conserving_mole_equivariance(dtype,num_tol,rot_tol,conserving_mole_checkpoint,fake_puma_dataset):
    conserving_mole_inference_checkpoint_pt, _ = conserving_mole_checkpoint
    equivariance_on_pt(dtype,num_tol,rot_tol,conserving_mole_inference_checkpoint_pt,fake_puma_dataset)



def equivariance_on_pt(dtype,num_tol,rot_tol,inference_checkpoint_path,data_root_dir):
    db=AseDBDataset(
                config={"src": os.path.join(data_root_dir,'oc20')}
            )
    ...
    for sample_idx in range(5):
        ...
        print(f"numerical test , {dtype} , energy_var: {energy_var}, force_var:{force_var}")
        ...
        print(f"equivariance test , {dtype} , energy_var: {energy_var}, force_var:{force_var}")
