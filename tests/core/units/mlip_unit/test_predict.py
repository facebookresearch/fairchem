...
# ---------------------------------------------------------------------------
# Unit tests for single-atom helper functions (CPU only, no model required)
# ---------------------------------------------------------------------------


class TestIdentifySingleAtomSystems:
    """Unit tests for identify_single_atom_systems function."""

    def test_mixed_batch(self):
        """Test identification of single atoms in a mixed batch."""
        from ase import Atoms

        from fairchem.core.units.mlip_unit.predict import identify_single_atom_systems

        atoms_list = [
            Atoms("C", positions=[[0, 0, 0]], pbc=False),  # single, no pbc
            Atoms(
                "CO2",
            ...
            Atoms("O", positions=[[0, 0, 0]], pbc=False),  # single, no pbc
            Atoms(
                "H2", positions=[[0, 0, 0], [0.75, 0, 0]], pbc=True, cell=[10, 10, 10]
            ),  # multi
        ]
        data_list = [AtomicData.from_ase(a, task_name="test") for a in atoms_list]
        data = atomicdata_list_to_batch(data_list)

        single_mask, regular_mask = identify_single_atom_systems(data)

        assert single_mask.tolist() == [True, False, True, False]
        assert regular_mask.tolist() == [False, True, False, True]

    def test_single_atom_with_pbc_is_not_single(self):
        """A single atom with PBC should not be identified as isolated."""
        from ase import Atoms

        from fairchem.core.units.mlip_unit.predict import identify_single_atom_systems

        # Single atom but with periodic boundary conditions
        atoms = Atoms("Cu", positions=[[0, 0, 0]], pbc=True, cell=[3.6, 3.6, 3.6])
        data = AtomicData.from_ase(atoms, task_name="test")

        single_mask, regular_mask = identify_single_atom_systems(data)

        assert single_mask.tolist() == [False]
        assert regular_mask.tolist() == [True]

    def test_all_single_atoms(self):
        """Test batch with all single atoms."""
        from ase import Atoms

        from fairchem.core.units.mlip_unit.predict import identify_single_atom_systems

        atoms_list = [
            Atoms("C", positions=[[0, 0, 0]], pbc=False),
            Atoms("O", positions=[[0, 0, 0]], pbc=False),
        ]
        data_list = [AtomicData.from_ase(a, task_name="test") for a in atoms_list]
        data = atomicdata_list_to_batch(data_list)
        ...
        assert single_mask.all()
        assert not regular_mask.any()


class TestExtractSystemsByMask:
    """Unit tests for extract_systems_by_mask function."""

    def test_extract_subset(self):
        """Test extracting a subset of systems from a batch."""
        from ase import Atoms

        from fairchem.core.units.mlip_unit.predict import extract_systems_by_mask
        ...
        # Extract only systems 0 and 2
        mask = torch.tensor([True, False, True])
        extracted = extract_systems_by_mask(data, mask)

        assert len(extracted) == 2
        assert extracted.natoms.tolist() == [1, 1]

    def test_extract_multi_atom_systems(self):
        """Test extracting multi-atom systems preserves atom counts."""
        from ase import Atoms
        ...
        # Extract only multi-atom systems (indices 1 and 2)
        mask = torch.tensor([False, True, True])
        extracted = extract_systems_by_mask(data, mask)

        assert len(extracted) == 2
        assert extracted.natoms.tolist() == [3, 2]
        assert extracted.natoms.sum().item() == 5


class TestInterleavePredictions:
    """Unit tests for interleave_predictions function."""

    def test_mixed_batch_energy_ordering(self):
        """Test that energies are correctly interleaved in original order."""
        from types import SimpleNamespace

        from ase import Atoms

        from fairchem.core.units.mlip_unit.predict import (
            identify_single_atom_systems,
            interleave_predictions,
        )
        ...
        single_mask, _ = identify_single_atom_systems(data)
        tasks = {"energy": SimpleNamespace(level="system", property="energy")}

        single_preds = {"energy": torch.tensor([-10.0, -20.0])}  # systems 0, 2
        regular_preds = {"energy": torch.tensor([-100.0, -200.0])}  # systems 1, 3

        merged = interleave_predictions(
            single_preds, regular_preds, single_mask, tasks, data
        )

        expected = torch.tensor([-10.0, -100.0, -20.0, -200.0])
        assert torch.allclose(merged["energy"], expected)

    def test_mixed_batch_forces_placement(self):
        """Test that forces are correctly placed for each system's atoms."""
        from types import SimpleNamespace
        ...
        # System 0: 1 atom, System 1: 3 atoms, System 2: 1 atom, System 3: 2 atoms
        atoms_list = [
            Atoms("C", positions=[[0, 0, 0]], pbc=False),
            Atoms(
                "CO2",
        ...
        single_mask, _ = identify_single_atom_systems(data)
        tasks = {"forces": SimpleNamespace(level="atom", property="forces")}

        # Single atom forces (2 atoms total)
        single_preds = {"forces": torch.zeros(2, 3)}

        # Regular system forces (3 + 2 = 5 atoms total)
        regular_forces = torch.randn(5, 3)
        regular_preds = {"forces": regular_forces}

        merged = interleave_predictions(
            single_preds, regular_preds, single_mask, tasks, data
        )

        # Total should be 7 atoms
        assert merged["forces"].shape == (7, 3)

        # System 0 (atom 0): single atom -> zeros
        assert torch.allclose(merged["forces"][0], torch.zeros(3))

        # System 1 (atoms 1-3): regular -> first 3 from regular_forces
        assert torch.allclose(merged["forces"][1:4], regular_forces[:3])

        # System 2 (atom 4): single atom -> zeros
        assert torch.allclose(merged["forces"][4], torch.zeros(3))

        # System 3 (atoms 5-6): regular -> last 2 from regular_forces
        assert torch.allclose(merged["forces"][5:7], regular_forces[3:5])

    def test_all_single_atoms_returns_directly(self):
        """When regular_preds is None, should return single_preds directly."""
        from types import SimpleNamespace
        ...
        single_preds = {"energy": torch.tensor([-10.0, -20.0])}

        result = interleave_predictions(single_preds, None, single_mask, tasks, data)

        # Should return the same dict object
        assert result is single_preds
