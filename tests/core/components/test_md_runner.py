...
class TestMDRunner:
    ...
    def test_langevin_checkpoint_resume(self, cu_atoms, results_dir):
        """
        Test checkpoint/resume with Langevin stochastic dynamics.

        Verifies:
        1. Langevin RNG state is saved
        2. State is correctly restored on resume
        3. Trajectory steps are aligned across runs
        """
        results_dir1 = results_dir / "results1"
        ...
        with open(checkpoint_dir / "thermostat_state.json") as f:
            saved_state = json.load(f)
        assert saved_state["class_name"] == "Langevin"
        assert "rng_state" in saved_state
        assert "transferred_energy" not in saved_state
