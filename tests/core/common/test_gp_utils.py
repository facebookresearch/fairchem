...
def simple_layer(x, edge_index, node_offset, natoms, n=3):
    ...
    if gp_utils.initialized():
        ...
        local_atoms=size_list_fn(natoms, gp_utils.get_gp_world_size())[gp_utils.get_gp_rank()]
    else:
...
class SimpleNet(nn.Module):
    ...
    def forward(self, atomic_numbers, edge_index):
        ...
        if gp_utils.initialized():
            local_atoms=size_list_fn(graph_dict['natoms'], gp_utils.get_gp_world_size())[gp_utils.get_gp_rank()]
            final_node_embeddings=final_node_embeddings[graph_dict["node_offset"]:graph_dict["node_offset"]+local_atoms ]
