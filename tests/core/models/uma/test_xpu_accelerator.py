...
def _cu_batch(device: str, sizes=(1, 2)):
    """A small periodic metal system, batched at two sizes."""
    samples = []
    for size in sizes:
        atoms = build.bulk("Cu", "fcc", a=3.58, cubic=True).repeat((size,) * 3)
        sample = AtomicData.from_ase(atoms, max_neigh=300, radius=CUTOFF, r_edges=True)
...
def test_accelerator_matches_cpu_within_tolerance():
    ...
    assert torch.allclose(
        e_cpu, e_acc, atol=1e-3, rtol=1e-3
