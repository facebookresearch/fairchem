...
def test_node_to_edge_wigner_permute_matches_pytorch(sphere_channels):
    ...
    # Triton kernel
    triton_out = node_to_edge_wigner_permute_launcher(x, edge_index, wigner)
...
def test_permute_wigner_inv_matches_pytorch(sphere_channels):
    ...
    # Triton kernel
    triton_out = permute_wigner_inv_edge_to_node_launcher(x, wigner_inv)
...
def test_permute_wigner_inv_bwd_dw_matches_pytorch(sphere_channels):
    ...
    The kernel expects M-major x_m input and permutes to L-major internally.
    """
    torch.manual_seed(42)
    device = "cuda"
    num_edges = 32

    # Create inputs: grad_out in L-major, x_m in M-major
    grad_out = torch.randn(num_edges, 9, sphere_channels, device=device)
    x_m = torch.randn(num_edges, 9, sphere_channels, device=device)

    # Compute x_l (L-major) from x_m for the PyTorch reference
    x_l = x_m[:, M_TO_L_GATHER_IDX, :]

    # PyTorch reference: block-diagonal outer product using L-major data
    ...
    # Triton kernel via custom op — pass M-major x_m (kernel permutes internally)
