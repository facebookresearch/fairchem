...
# =============================================================================
# Test gradient equivariance
# =============================================================================


class TestGradientEquivariance:
    """Tests demonstrating that quaternion approach preserves gradient equivariance.

    For a proper equivariant model, gradients should have consistent magnitude
    regardless of edge orientation. The Euler approach breaks this by suppressing
    gradients for y-aligned edges (due to Safeacos clamping).
    """

    def test_gradient_equivariance_y_vs_z(self, lmax, dtype, device, wigner_coeffs, U_matrix, Jd_matrices):
        """Compare gradients for y-aligned vs z-aligned edges.

        For a "structure" with edges along different axes, the gradient magnitudes
        should be similar if the Wigner D computation is equivariant. This test shows:

        - Z-aligned Euler: finite (no singularity at z-axis)
        - Z-aligned Quaternion: finite (correct)
        - Y-aligned Quaternion: finite (correct, preserves equivariance)
        - Y-aligned Euler: ~0 (BROKEN - gradient completely suppressed by Safeacos)

        Note: Absolute magnitudes may differ due to gamma handling, but the key
        insight is that Euler completely suppresses Y-aligned gradients while
        quaternion preserves them.
        """
        # Create z-aligned edge (no singularity for Euler)
        z_edge = torch.tensor([[0.0, 0.0, 1.0]], dtype=dtype, device=device)

        # Create y-aligned edge (singularity for Euler)
        y_edge = torch.tensor([[0.0, 1.0, 0.0]], dtype=dtype, device=device)

        results = {}

        # Z-aligned edge with Euler approach
        edge = z_edge.clone().requires_grad_(True)
        euler_angles = init_edge_rot_euler_angles(edge)
        wigner = eulers_to_wigner(euler_angles, 0, lmax, Jd_matrices)
        loss = wigner.sum()
        loss.backward()
        results["z_euler"] = edge.grad.abs().max().item()

        # Z-aligned edge with quaternion approach
        edge = z_edge.clone().requires_grad_(True)
        wigner, _ = get_wigner_from_edge_vectors(edge, lmax, wigner_coeffs, U_matrix)
        loss = wigner.sum()
        loss.backward()
        results["z_quat"] = edge.grad.abs().max().item()

        # Y-aligned edge with quaternion approach
        edge = y_edge.clone().requires_grad_(True)
        wigner, _ = get_wigner_from_edge_vectors(edge, lmax, wigner_coeffs, U_matrix)
        loss = wigner.sum()
        loss.backward()
        results["y_quat"] = edge.grad.abs().max().item()

        # Y-aligned edge with Euler approach
        edge = y_edge.clone().requires_grad_(True)
        euler_angles = init_edge_rot_euler_angles(edge)
        wigner = eulers_to_wigner(euler_angles, 0, lmax, Jd_matrices)
        loss = wigner.sum()
        loss.backward()
        results["y_euler"] = edge.grad.abs().max().item()

        # Print results for visibility
        print("\n" + "=" * 60)
        print("Gradient Equivariance Test Results")
        print("=" * 60)
        print(f"Z-aligned edge (Euler):     {results['z_euler']:12.6f}")
        print(f"Z-aligned edge (Quaternion):{results['z_quat']:12.6f}")
        print(f"Y-aligned edge (Quaternion):{results['y_quat']:12.6f}")
        print(f"Y-aligned edge (Euler):     {results['y_euler']:12.6f}  <- SUPPRESSED!")
        print("=" * 60)

        # Verify equivariance for quaternion approach:
        # Both Y and Z aligned edges should have finite, non-zero gradients
        quat_ratio = results["y_quat"] / results["z_quat"]
        print(f"\nQuaternion Y/Z ratio: {quat_ratio:.3f}")

        # Verify Euler approach breaks equivariance:
        # Y-aligned gradient is suppressed relative to Z-aligned
        if results["z_euler"] > 0:
            euler_ratio = results["y_euler"] / results["z_euler"]
            print(f"Euler Y/Z ratio:      {euler_ratio:.6f} (<<1 shows broken equivariance)")
        else:
            euler_ratio = 0.0
            print(f"Euler Y/Z ratio:      N/A (z_euler is 0)")

        # Assertions
        # 1. Quaternion approach should give finite gradients for BOTH orientations
        assert results["y_quat"] > 0.1, \
            f"Quaternion y-gradient should be finite, got {results['y_quat']}"
        assert results["z_quat"] > 0.1, \
            f"Quaternion z-gradient should be finite, got {results['z_quat']}"

        # 2. Euler approach should give finite gradient for z, but suppressed for y
        assert results["z_euler"] > 0.1, \
            f"Euler z-gradient should be finite, got {results['z_euler']}"
        assert results["y_euler"] < 0.01, \
            f"Euler y-gradient should be suppressed, got {results['y_euler']}"

        # 3. The suppression ratio should be dramatic for Euler
        assert euler_ratio < 0.01, \
            f"Euler y/z ratio should be <<1, got {euler_ratio}"
