...
# =========================================================================
# Distributed tests: overlap correctness
# =========================================================================


def overlap_a2a_test(atomic_numbers, edge_index):
    """
    Verify that overlapped all-to-all (local edges during comm,
    boundary edges after) produces the same results as sequential
    all-to-all (all edges after comm).
    """
    rank = gp_utils.get_gp_rank()
    world_size = gp_utils.get_gp_world_size()
    natoms = atomic_numbers.shape[0]
    ...
    # Filter edges: keep only edges where target is in our partition
    local_atom_mask = rank_assignments == rank
    target_in_partition = local_atom_mask[edge_index[1]]
    filtered_edge_index = edge_index[:, target_in_partition]

    gp_ctx = build_gp_context(filtered_edge_index, rank_assignments, rank, world_size)
    ...
    # --- Sequential: all-to-all then compute all edges ---
    x_received_seq = all_to_all_collect(x, gp_ctx, send_indices)
    x_full_seq = torch.cat([x, x_received_seq], dim=0)

    # Simple scatter-add message passing
    edge_features_seq = x_full_seq[edge_index_local[0]]
    result_seq = torch.zeros(local_atoms, 1, dtype=torch.float)
    result_seq.index_add_(0, edge_index_local[1], edge_features_seq)

    # --- Overlapped: local edges during comm, boundary after ---
    ...
    # Compute local edges (only need local atoms)
    edge_features_local = x[local_edge_idx[0]]
    result_local = torch.zeros(local_atoms, 1, dtype=torch.float)
    result_local.index_add_(0, local_edge_idx[1], edge_features_local)

    # Wait for comm
    x_received_ov = finish_all_to_all_collect(recv_buf, work_handles)
    x_full_ov = torch.cat([x, x_received_ov], dim=0)

    # Compute boundary edges
    edge_features_boundary = x_full_ov[boundary_edge_idx[0]]
    result_boundary = torch.zeros(local_atoms, 1, dtype=torch.float)
    result_boundary.index_add_(0, boundary_edge_idx[1], edge_features_boundary)

    # Sum contributions
    result_overlap = result_local + result_boundary

    match = torch.allclose(result_seq, result_overlap, atol=1e-6)
    return {
        "rank": rank,
        "match": match,
        "seq_result": result_seq.detach(),
        "overlap_result": result_overlap.detach(),
        "num_local_edges": gp_ctx.num_local_edges,
        "num_boundary_edges": gp_ctx.num_boundary_edges,
    }


def test_overlap_matches_sequential():
    """
    Verify that overlapped local+boundary edge computation produces
    the same results as sequential computation of all edges.
    """
    num_atoms = 8
    # Create a ring graph with extra cross edges
    src = []
    dst = []
    for i in range(num_atoms):
        for j in range(num_atoms):
    ...
    for result in all_rank_results:
        assert result["match"], (
            f"Rank {result['rank']}: overlap result doesn't match sequential. "
            f"local_edges={result['num_local_edges']}, "
            f"boundary_edges={result['num_boundary_edges']}"
        )
        # Verify there are BOTH local and boundary edges (otherwise the test is trivial)
        assert (
            result["num_local_edges"] > 0
        ), f"Rank {result['rank']}: no local edges — test is trivial"
        assert (
