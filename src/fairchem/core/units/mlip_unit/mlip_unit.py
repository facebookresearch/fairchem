...
def get_dataset_masks(batch: AtomicData) -> dict[str, torch.Tensor]:
    """One boolean mask over the systems of the batch, per dataset in it.

    Built once per batch so that per-task mask construction issues no host to
    device copies of its own.
    """
...
def get_output_mask(
    ...
    for dset, dset_mask in dataset_masks.items():
        if task.level == "atom":
            atom_mask = torch.repeat_interleave(
                dset_mask, batch.natoms, output_size=batch.pos.shape[0]
            )
            output_masks[f"{dset}.{task.name}"] = atom_mask & output_masks[task.name]
...
def compute_loss(
    ...
    # a static shape, not batch.natoms.sum(): a device scalar used as a view
    # size is read back to the host
