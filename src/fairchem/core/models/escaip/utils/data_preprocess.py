...
def get_global_attention_mask(
    node_batch: torch.Tensor, num_global_tokens: int, num_graphs: int
):
    # mask for global -> node cross attention
    # (num_graphs * num_global_tokens, 1, num_nodes)

    # (num_graphs, num_nodes) bool - valid node for each graph
    valid_mask = node_batch.unsqueeze(0) == torch.arange(
        num_graphs, device=node_batch.device
    ).unsqueeze(1)

    # repeat for each global token (num_graphs * num_global_tokens, num_nodes)
    global_node_mask = valid_mask.repeat_interleave(num_global_tokens, dim=0)

    # fill with -inf
    global_node_mask = torch.where(
        global_node_mask,
        torch.zeros(1, device=node_batch.device),
        torch.finfo(torch.float32).min,
    ).unsqueeze(1)

    # mask for node -> global cross attention
    # (1, num_nodes, num_graphs * num_global_tokens)

    # (num_graphs * num_global_tokens) bool - id of graph token for each graph
    graph_token_ids = torch.arange(
        num_graphs, device=node_batch.device
    ).repeat_interleave(num_global_tokens)

    # valid graph token for each node
    valid_graph_token_mask = (
        node_batch.unsqueeze(1) == graph_token_ids.unsqueeze(0)
    ) & (node_batch.unsqueeze(1) != -1)

    # fill with -inf
    node_global_mask = torch.where(
        valid_graph_token_mask,
        torch.zeros(1, device=node_batch.device),
        torch.finfo(torch.float32).min,
    ).unsqueeze(0)

    return global_node_mask, node_global_mask
...
def data_preprocess_radius_graph(
    ...
    # get attention mask for global attention
    global_node_mask, node_global_mask = get_global_attention_mask(
        node_batch, gnn_cfg.num_global_tokens, max_batch_size
    )
    ...
    x = GraphAttentionData(
        ...
        node_global_mask=node_global_mask,
        neighbor_index=neighbor_index,
