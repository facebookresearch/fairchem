...
def build_gp_context(
    ...
    # send_info is computed during graph filtering in
    # filter_edges_by_node_partition, which has access to the full
    # (pre-filter) edge_index needed to derive send metadata locally.
    ...
    if n_bad > 0:
        ...
        logging.error(
            f"Rank {rank}: {n_bad} entries in edge_index_local "
            f"are -1 (unmapped). "
            f"edge_needed={edge_needed_count}, "
            f"total_needed={total_needed_atoms}, "
        ...
        raise RuntimeError(
            ...
            f"global_to_local mapping. Some remote atoms needed by "
            f"local edges were not discovered during context building."
        )
