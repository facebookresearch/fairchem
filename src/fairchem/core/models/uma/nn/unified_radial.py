...
We run one batched computation:
    all_radial_outs = unified_radial_mlp(x_edge)  # [E, num_layers, out]
"""
...
class UnifiedRadialMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        return outputs  # list of N tensors, each [E, out]

    def forward_single(self, x: torch.Tensor, layer_idx: int) -> torch.Tensor:
        """
        Compute radial output for a single layer (for validation/debugging).

        Args:
            x: Input tensor of shape [E, in_features]
            layer_idx: Which layer's radial function to compute

        Returns:
            Output tensor of shape [E, out_features]
        """
        H = self.hidden_features
        # Split concatenated W1_cat/b1_cat into per-layer weights
        W1_per_layer = self.W1_cat.split(H, dim=0)
        b1_per_layer = self.b1_cat.split(H, dim=0)

        h = torch.nn.functional.linear(
            x, W1_per_layer[layer_idx], b1_per_layer[layer_idx]
        )
        h = torch.nn.functional.layer_norm(
            h,
            (H,),
            self.gamma1[layer_idx],
            self.beta1[layer_idx],
            self.ln_eps,
        )
        h = torch.nn.functional.silu(h)

        h = torch.nn.functional.linear(h, self.W2[layer_idx], self.b2[layer_idx])
        h = torch.nn.functional.layer_norm(
            h,
            (H,),
            self.gamma2[layer_idx],
            self.beta2[layer_idx],
            self.ln_eps,
        )
        h = torch.nn.functional.silu(h)

        return torch.nn.functional.linear(h, self.W3[layer_idx], self.b3[layer_idx])
...
def create_unified_radial_mlp(
    ...
    Args:
        ...
        backend: Which implementation to use (only "einsum" supported)

    Returns:
        Unified radial MLP module
    """
    if backend != "einsum":
        raise ValueError(f"Unknown backend: {backend}. Only 'einsum' is supported.")
    return UnifiedRadialMLP(rad_funcs)


class UnifiedRadialMLPLayer1Only(nn.Module):
    """
    Simpler unified radial MLP that ONLY fuses Layer 1.

    Layer 1: Fused single GEMM (all rad_funcs share same input)
    Layers 2-3: Delegate to original rad_func modules

    Benefits:
    - Works with heterogeneous output sizes (no W3 shape issues)
    - Simpler implementation
    - Same Layer 1 fusion benefit
    """
    ...
    def __init__(self, radial_mlps: list[RadialMLP]) -> None:
        ...
        # Keep references to original modules for layers 2-3
        self.radial_mlps = nn.ModuleList(radial_mlps)

        # Validate architecture
        first_mlp = radial_mlps[0]
        if len(first_mlp.net) != 7:
            raise ValueError(
                f"Expected 7 modules in RadialMLP, got " f"{len(first_mlp.net)}"
            )

        self.in_features = first_mlp.net[0].in_features
        self.hidden_features = first_mlp.net[0].out_features

        # Layer 1 ONLY: Concatenate all weights into one big linear
        W1_list = [mlp.net[0].weight.data for mlp in radial_mlps]
        b1_list = [mlp.net[0].bias.data for mlp in radial_mlps]
        self.register_buffer("W1_cat", torch.cat(W1_list, dim=0))
        self.register_buffer("b1_cat", torch.cat(b1_list, dim=0))
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        Layer 1: Fused single GEMM
        Layers 2-3: Use original rad_func modules
        """
        N = self.num_layers
        H = self.hidden_features

        # Layer 1: Single big GEMM (FUSED)
        h_all = torch.nn.functional.linear(x, self.W1_cat, self.b1_cat)
        h_all = h_all.view(h_all.shape[0], N, H)

        # Layers 2-3: Use original rad_func.net[1:]
        # (LayerNorm -> SiLU -> Linear -> LN -> SiLU -> Linear)
        outputs = []
        for i in range(N):
            h = h_all[:, i]  # [E, hidden]
            # net[1:] = LayerNorm, SiLU, Linear, LayerNorm, SiLU, Linear
            out = self.radial_mlps[i].net[1:](h)
            outputs.append(out)

        return outputs


def create_unified_radial_mlp_layer1_only(
    rad_funcs: list[RadialMLP],
) -> UnifiedRadialMLPLayer1Only:
    """
    Factory function for Layer1-only unified radial MLP.

    Works with heterogeneous output sizes (e.g., SO2_Conv + EdgeDegree).
    """
    return UnifiedRadialMLPLayer1Only(rad_funcs)
