...
from .fused_edge_wigner_fwd import fused_edge_gather_wigner_l2m_lmax2
...
# M_TO_L_GATHER_IDX for reference
M_TO_L_GATHER_IDX = [0, 5, 1, 3, 8, 6, 2, 4, 7]
...
BackendType = Literal["pytorch", "triton", "scatter_add", "recompute"]
...
def fused_edge_gather_wigner_l2m_lmax2_with_bwd(
    x: torch.Tensor,
    edge_index: torch.Tensor,
    wigner: torch.Tensor,
    backward_impl: BackendType = "triton",
) -> torch.Tensor:
    """Fused edge gather + Wigner + L→M with configurable backward.

    Drop-in replacement that allows selecting between Triton and PyTorch
    backward implementations.

    Args:
        x: Node features [N, 9, C] in L-major order
        edge_index: Edge indices [2, E]
        wigner: Per-edge Wigner matrices [E, 9, 9] or [E, 81]
        backward_impl:
            - "triton": atomic-based Triton backward (default)
            - "scatter_add": two-phase (no atomics) Triton backward
            - "pytorch": PyTorch backward
            - "recompute": memory-optimized, recomputes edge features in backward
              (saves ~670MB for typical 2000 node, 74K edge graphs)

    Returns:
        Edge features [E, 9, 2C] in M-major order
    """
    if backward_impl == "triton":
        return FusedEdgeGatherWignerL2MTritonBwdFunction.apply(x, edge_index, wigner)
    elif backward_impl == "scatter_add":
        return FusedEdgeGatherWignerL2MTritonV2BwdFunction.apply(x, edge_index, wigner)
    elif backward_impl == "pytorch":
        return FusedEdgeGatherWignerL2MPyTorchBwdFunction.apply(x, edge_index, wigner)
    elif backward_impl == "recompute":
        return FusedEdgeGatherWignerL2MRecomputeFunction.apply(x, edge_index, wigner)
    else:
        raise ValueError(
            f"Unknown backward_impl: {backward_impl}. Use 'triton', 'scatter_add', 'pytorch', or 'recompute'."
        )
...
def fused_edge_gather_wigner_triton_bwd(
    x: torch.Tensor,
    edge_index: torch.Tensor,
    wigner: torch.Tensor,
) -> torch.Tensor:
    """Alias using Triton backward."""
    return fused_edge_gather_wigner_l2m_lmax2_with_bwd(
        x, edge_index, wigner, backward_impl="triton"
    )


def fused_edge_gather_wigner_pytorch_bwd(
    x: torch.Tensor,
    edge_index: torch.Tensor,
    wigner: torch.Tensor,
) -> torch.Tensor:
    """Alias using PyTorch backward."""
    return fused_edge_gather_wigner_l2m_lmax2_with_bwd(
        x, edge_index, wigner, backward_impl="pytorch"
    )
