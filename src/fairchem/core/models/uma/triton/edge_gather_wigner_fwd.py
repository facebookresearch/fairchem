...
@triton.jit
def fused_edge_gather_wigner_l2m_kernel(
    x_ptr,
    edge_index_ptr,
    wigner_ptr,
    out_ptr,
    ...
    Equivalent to:
        x_edge = cat([x[idx0], x[idx1]], dim=2)  # [E, 9, 2C]
        x_rot = block_diagonal_wigner(x_edge, wigner)  # [E, 9, 2C]
        x_m = permute_L_to_M(x_rot)  # [E, 9, 2C]

    Exploits block-diagonal sparsity:
      L=0: 1x1 block, positions [0]
      L=1: 3x3 block, positions [1,2,3]
      L=2: 5x5 block, positions [4,5,6,7,8]

    Then permutes to M-major ordering for SO2 convolution.

    Uses 2D grid: (num_edges, num_c_blocks) to handle channels > BLOCK_C.
    """
    ...
    # =========================================================================
    # Helper: Load x[node, coeff, :] for both source and target nodes
    # Returns concatenated [x_src, x_tgt] as 2*BLOCK_C accumulator
    # =========================================================================

    # Load all 9 coefficients from both nodes (will reuse across blocks)
    x0_src = tl.load(
    ...
    # =========================================================================
    # L→M permutation and store using output strides
    # L_TO_M_GATHER_IDX = [0, 2, 6, 3, 7, 1, 5, 8, 4]
    # Note: c_range already includes c_start offset from 2D grid
    # out[i] = y[L_TO_M_GATHER_IDX[i]]
    # =========================================================================
    # M-pos 0 <- L-pos 0
    tl.store(
...
def fused_edge_gather_wigner_l2m_lmax2(
    x: torch.Tensor,
    edge_index: torch.Tensor,
    wigner: torch.Tensor,
) -> torch.Tensor:
    """Fused edge gather + block-diagonal Wigner rotation + L→M permutation.

    Equivalent to:
        x_edge = torch.cat([x[edge_index[0]], x[edge_index[1]]], dim=2)
        x_rot = block_diagonal_wigner(x_edge, wigner)  # using sparsity
        x_m = permute_L_to_M(x_rot)
    ...
    Returns:
        Rotated edge features [num_edges, 9, 2*sphere_channels] in M-major ordering
    """
    ...
    fused_edge_gather_wigner_l2m_kernel[(num_edges, num_c_blocks)](
        x,
        edge_index,
        wigner_flat,
        out,
