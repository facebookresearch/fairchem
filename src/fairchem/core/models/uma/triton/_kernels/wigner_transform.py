...
Triton kernels for Wigner D-matrix transforms (lmax=2).
...
Forward pass (fused M→L + Wigner):
    x_m [E, 9, C]  →  permute M→L  →  x_l [E, 9, C]  →  W @ x_l  →  y_l [E, 9, C]

Backward pass (fused Wigner + L→M):
...
# Tensor dimensions
NUM_COEFFS = 9  # Number of spherical harmonic coefficients for lmax=2
WIGNER_SIZE = NUM_COEFFS * NUM_COEFFS  # 81 = flattened 9x9 Wigner matrix
BLOCK_C = 128  # Channel block size for Triton kernels (fixed for UMA)

# Block sizes for each L
L0_SIZE = 1  # L=0 block: 1x1
L1_SIZE = 3  # L=1 block: 3x3
L2_SIZE = 5  # L=2 block: 5x5

# Starting indices in the 9-element coefficient vector
L0_START = 0  # L=0 starts at index 0
L1_START = 1  # L=1 starts at index 1
L2_START = 4  # L=2 starts at index 4

# L→M permutation: out_m[i] = in_l[L_TO_M_GATHER_IDX[i]]
L_TO_M_GATHER_IDX = [0, 2, 6, 3, 7, 1, 5, 8, 4]

# M→L permutation: out_l[i] = in_m[M_TO_L_GATHER_IDX[i]]
...
if HAS_TRITON:
    ...
    # y = W @ x where W is block-diagonal
    #
    # Reference (PyTorch equivalent):
    #   y[:, 0, :] = W[:, 0, 0:1] @ x[:, 0:1, :]       # L=0: 1x1
    #   y[:, 1:4, :] = W[:, 1:4, 1:4] @ x[:, 1:4, :]   # L=1: 3x3
    #   y[:, 4:9, :] = W[:, 4:9, 4:9] @ x[:, 4:9, :]   # L=2: 5x5
    # =========================================================================
    ...
    def wigner_lmax2_fwd_kernel(
        ...
        """
        Block-diagonal Wigner multiply for lmax=2: y = W @ x
        ...
        # Channel range for vectorized load/store (BLOCK_C=128)
        c_range = tl.arange(0, 128)
        c_mask = c_range < C

        # Base offsets (WIGNER_SIZE=81, NUM_COEFFS=9)
        w_base = edge_id * 81  # W[edge_id, :, :] flattened
        x_base = edge_id * 9 * C  # X[edge_id, :, :]
        out_base = edge_id * 9 * C

        # -----------------------------------------------------------------
        # L=0 BLOCK (1x1): y[0] = W[0,0] * x[0]
        # -----------------------------------------------------------------
        ...
        # -----------------------------------------------------------------
        # L=1 BLOCK (3x3): y[1:4] = W[1:4,1:4] @ x[1:4]
        # -----------------------------------------------------------------
        ...
        # -----------------------------------------------------------------
        # L=2 BLOCK (5x5): y[4:9] = W[4:9,4:9] @ x[4:9]
        # -----------------------------------------------------------------
    ...
    #
    # Reference (PyTorch equivalent):
    #   dx[:, 0, :] = W[:, 0, 0:1].T @ dy[:, 0:1, :]     # L=0: 1x1
    #   dx[:, 1:4, :] = W[:, 1:4, 1:4].T @ dy[:, 1:4, :] # L=1: 3x3
    #   dx[:, 4:9, :] = W[:, 4:9, 4:9].T @ dy[:, 4:9, :] # L=2: 5x5
    # =========================================================================
    ...
    def wigner_lmax2_bwd_dx_kernel(
        ...
        # -----------------------------------------------------------------
        # L=0 BLOCK (1x1): dx[0] = W[0,0]^T * dy[0] = W[0,0] * dy[0]
        # -----------------------------------------------------------------
        w00 = tl.load(W_ptr + w_base + 0)
        ...
        # -----------------------------------------------------------------
        # L=1 BLOCK (3x3): dx[1:4] = W[1:4,1:4]^T @ dy[1:4]
        ...
        # -----------------------------------------------------------------
        # L=2 BLOCK (5x5): dx[4:9] = W[4:9,4:9]^T @ dy[4:9]
        # -----------------------------------------------------------------
    ...
    # dW = dy @ x^T (outer product, reduced over channels)
    #
    # Reference (PyTorch equivalent per edge):
    #   dW[e, 0, 0] = (dy[e, 0, :] * x[e, 0, :]).sum()          # L=0
    #   dW[e, 1:4, 1:4] = dy[e, 1:4, :] @ x[e, 1:4, :].T        # L=1
    #   dW[e, 4:9, 4:9] = dy[e, 4:9, :] @ x[e, 4:9, :].T        # L=2
    #
    # Required for force computation (gradients flow through Wigner matrices)!
    # =========================================================================
    ...
    def wigner_lmax2_bwd_dw_kernel(
        ...
        """
        Backward for Wigner: dW[i,j] = sum_c dy[i,c] * x[j,c]
        ...
        # -----------------------------------------------------------------
        # L=1 BLOCK (3x3): dW[1:4, 1:4] = outer product reduced over C
        # -----------------------------------------------------------------
        dw_11 = tl.sum(dy1 * x1)
        dw_12 = tl.sum(dy1 * x2)
        dw_13 = tl.sum(dy1 * x3)
        ...
        # -----------------------------------------------------------------
        # L=2 BLOCK (5x5): dW[4:9, 4:9] = outer product reduced over C
        # -----------------------------------------------------------------
        # Row 4
        dw_44 = tl.sum(dy4 * x4)
        dw_45 = tl.sum(dy4 * x5)
    ...
    # Reorder coefficients from L-major to M-major ordering
    #
    # L-major: [L=0, L=1 m=-1,0,+1, L=2 m=-2,-1,0,+1,+2]
    # M-major: [m=0 (L=0,1,2), m=+1 (L=1,2), m=-1 (L=1,2), m=+2, m=-2]
    #
    # Reference (PyTorch): out[:, i] = x[:, L_TO_M_GATHER_IDX[i]]
    # =========================================================================
    ...
    # Reorder coefficients from M-major to L-major ordering (inverse of L->M)
    #
    # M-major: [m=0 (L=0,1,2), m=+1 (L=1,2), m=-1 (L=1,2), m=+2, m=-2]
    # L-major: [L=0, L=1 m=-1,0,+1, L=2 m=-2,-1,0,+1,+2]
    #
    # Reference (PyTorch): out[:, i] = x[:, M_TO_L_GATHER_IDX[i]]
    # =========================================================================
    ...
    #
    # Combines two operations in one kernel to avoid intermediate memory:
    #   1. Permute M-major → L-major (gather from M positions)
    #   2. Apply block-diagonal Wigner: y = W @ x_l
    #
    # Data flow: x_m [E, 9, C] → permute → x_l [E, 9, C] → W @ x_l → y_l [E, 9, C]
    #
    # Reference (PyTorch equivalent):
    #   x_l = x_m[:, M_TO_L_GATHER_IDX, :]  # permute
    #   y_l[:, 0] = W[:, 0, 0] * x_l[:, 0]  # L=0 block
    #   y_l[:, 1:4] = W[:, 1:4, 1:4] @ x_l[:, 1:4]  # L=1 block
    #   y_l[:, 4:9] = W[:, 4:9, 4:9] @ x_l[:, 4:9]  # L=2 block
    #
    ...
    def fused_m_to_l_wigner_lmax2_kernel(
        ...
        """
        Fused M->L permutation + block-diagonal Wigner multiply.

        Args:
            X_ptr: Input [E, 9, C] in M-major order
            W_ptr: Wigner matrix [E, 81]
            OUT_ptr: Output [E, 9, C] in L-major order (y = W @ x_l)
            XL_ptr: Optional save buffer for x_l [E, 9, C] (for backward dW)
            num_edges: Number of edges E
            sphere_channels: Number of channels C
            BLOCK_C: Channel block size (compile-time constant)
            SAVE_XL: Whether to save x_l for backward (compile-time constant)
    ...
    #
    # Combines two operations in one kernel to avoid intermediate memory:
    #   1. Apply transposed block-diagonal Wigner: dx_l = W^T @ dy
    #   2. Permute L-major → M-major (scatter to M positions)
    #
    # Data flow: dy_l [E, 9, C] → W^T @ dy → dx_l [E, 9, C] → permute → dx_m [E, 9, C]
    #
    # Reference (PyTorch equivalent):
    #   dx_l[:, 0] = W[:, 0, 0] * dy[:, 0]  # L=0 block (W^T = W for scalar)
    #   dx_l[:, 1:4] = W[:, 1:4, 1:4].T @ dy[:, 1:4]  # L=1 block
    #   dx_l[:, 4:9] = W[:, 4:9, 4:9].T @ dy[:, 4:9]  # L=2 block
    #   dx_m = dx_l[:, L_TO_M_GATHER_IDX, :]  # permute (actually scatter)
    #
    # Uses 2D grid (num_edges, num_c_blocks) to handle C > 128.
    # =========================================================================
    ...
    def fused_wigner_bwd_dx_l_to_m_kernel(
        ...
        """
        Fused Wigner backward dx + L->M permutation.

        Args:
            DY_ptr: Upstream gradient [E, 9, C] in L-major order
            W_ptr: Wigner matrix [E, 81]
            DX_ptr: Input gradient [E, 9, C] in M-major order
            num_edges: Number of edges E
            sphere_channels: Number of channels C
            BLOCK_C: Channel block size (compile-time constant)
