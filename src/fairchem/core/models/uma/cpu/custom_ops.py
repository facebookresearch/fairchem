"""
Register CPU Wigner kernels as torch custom ops for torch.compile compatibility.

Using torch.library.custom_op makes the kernels visible to torch.compile/dynamo,
avoiding graph breaks. This is the CPU equivalent of triton_op used for GPU.
"""
...
def _n2e_fwd(x: Tensor, edge_index: Tensor, wigner: Tensor) -> Tensor:
    ...
    return kernels.node_to_edge_wigner_permute_fwd(x, edge_index.to(torch.int64), wigner)
...
def _n2e_bwd_dx(grad_out: Tensor, wigner: Tensor, edge_index: Tensor, num_nodes: int) -> Tensor:
    kernels = _get_kernels()
    return kernels.node_to_edge_wigner_permute_bwd_dx(grad_out, wigner, edge_index, num_nodes)


@_n2e_bwd_dx.register_fake
def _n2e_bwd_dx_fake(grad_out: Tensor, wigner: Tensor, edge_index: Tensor, num_nodes: int) -> Tensor:
