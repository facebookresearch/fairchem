...
from fairchem.core.optim.optimizable import OptimizableBatch
...
class LBFGS:
    ...
    def determine_step(self, dr):
        ...
        num_systems = self.optimizable.batch.natoms.shape[0]
        longest_steps = torch.zeros(num_systems, dtype=steplengths.dtype, device=steplengths.device)
        longest_steps.scatter_reduce_(0, self.optimizable.batch_indices, steplengths, reduce="amax")
    ...
    def _batched_dot(self, x: torch.Tensor, y: torch.Tensor):
        ...
        # )
        dot_products = (x * y).sum(dim=-1)
        num_systems = self.optimizable.batch.natoms.shape[0]
        result = torch.zeros(num_systems, dtype=dot_products.dtype, device=dot_products.device)
        result.scatter_reduce_(0, self.optimizable.batch_indices, dot_products, reduce="sum")
        return result
