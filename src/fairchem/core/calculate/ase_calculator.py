...
class FAIRChemRayServeCalculator(Calculator):
    """
    ASE Calculator that uses Ray Serve inference server.
    
    This calculator submits inference requests to a centralized Ray Serve
    deployment. It transparently handles:
    - Model multiplexing (different checkpoints via model_id)
    - Automatic batching (Ray Serve handles this)
    - Scale-to-zero when idle (replicas spin down after timeout)
    
    This calculator is intended for use within Prefect tasks running on
    a Ray cluster with the FAIRChem inference server deployed.
    
    Args:
        model_id: Model identifier in format "checkpoint:task:settings".
            Examples:
            - "uma-s-1p1:omat" (checkpoint with task)
            - "uma-s-1p1:omat:default" (full specification)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.undo_element_references = undo_element_references
        self._client = None
    ...
    def from_preset(
        ...
        **kwargs,
        ...
        """
        Create calculator from preset configuration.
        
        Args:
            checkpoint: Model checkpoint name (e.g., "uma-s-1p1")
            task_name: Task name for UMA models (e.g., "omat", "omol", "omc")
            inference_settings: Inference settings ("default" or "turbo")
            **kwargs: Additional arguments passed to constructor
            
        Returns:
            FAIRChemRayServeCalculator instance
        """
        model_id = f"{checkpoint}:{task_name}:{inference_settings}"
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                from fairchem.workflows.serve import get_inference_client
            except ImportError as err:
                raise ImportError(
                    "fairchem.workflows is required for Ray Serve calculator. "
                    "Install with: pip install fairchem-workflows"
                ) from err
    ...
    def calculate(
        ...
        """
        Calculate properties for atoms.
        
        Submits inference request to Ray Serve deployment.
        """
        import numpy as np
        
        # Call parent to handle atoms setup
        Calculator.calculate(self, atoms, properties, system_changes)
        
        # Get predictions from serve
        client = self._get_client()
        result = client.predict(
            model_id=self.model_id,
            atoms=self.atoms,
            undo_element_references=self.undo_element_references,
        )
        
        # Map results to ASE format
        ...
        if "free_energy" in result and result["free_energy"] is not None:
            self.results["free_energy"] = float(result["free_energy"])
        elif "energy" in result:
            # Fall back to energy for free_energy
        ...
        if "stress" in result and result["stress"] is not None:
            ...
            # Ensure stress is in Voigt notation [xx, yy, zz, yz, xz, xy]
            if stress.shape == (3, 3):
                from ase.stress import full_3x3_to_voigt_6_stress
                self.results["stress"] = full_3x3_to_voigt_6_stress(stress)
            else:
                self.results["stress"] = stress.flatten()[:6]
