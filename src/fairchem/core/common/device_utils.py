...
say "the accelerator" instead of "cuda". It supports:
  ...
  * ``xpu``  -- Intel GPUs (Data Center GPU Max / Ponte Vecchio, e.g. Aurora and
    Sunspot), via ``torch.xpu`` + XCCL (oneCCL upstreamed into PyTorch).
...
#             init_process_group to register themselves. Recent ALCF frameworks
#             modules no longer ship them, so this is only a fallback.
...
def accelerator_is_available(device: str | torch.device) -> bool:
    ...
    except Exception:  # noqa: BLE001 - a broken backend must read as "absent"
        return False
...
def get_available_accelerator() -> str | None:
    ...
    if hasattr(torch, "accelerator"):
        try:
            ...
            if current is not None and current.type in ACCELERATOR_DEVICE_TYPES:
                if accelerator_is_available(current.type):
                    return current.type
...
def device_count(device: str | torch.device | None = None) -> int:
    ...
    device_type = device_type_of(device) if device is not None else get_available_accelerator()
...
def distributed_backend(device: str | torch.device) -> str:
    ...
    if backend is None:
        raise ValueError(f"no distributed backend known for device type {device_type!r}")
    if backend == "gloo":
        return backend
...
def max_memory_allocated(device: str | torch.device | None = None) -> int:
    ...
    if device_type is None or device_type == "cpu":
        return 0
    module = device_module(device_type)
    return module.max_memory_allocated() if hasattr(module, "max_memory_allocated") else 0
