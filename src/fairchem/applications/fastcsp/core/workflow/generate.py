...
def get_genarris_config(config: dict[str, Any]) -> dict[str, Any]:
    """Extract Genarris configuration from main config."""
...
def create_gnrs_submit_script(
    ...
    genarris_slurm_info: dict[str, Any],
    single_gnrs_folder: str | Path,
):
    """Create SLURM submit script for Genarris job."""

    slurm_script = "#!/bin/sh\n"
    for key, value in genarris_slurm_info.items():
...
{gnrs_config.get("mpi_launcher", "mpirun")} -np {genarris_slurm_info.get("nodes", 1) * genarris_slurm_info["ntasks-per-node"]} \\
...
def create_gnrs_config(
    ...
    """Create Genarris configuration file for specific molecule and parameters."""
    config = ConfigParser()
        ...
        for z in z_list:
            ...
            if not (single_gnrs_folder / "slurm.sh").exists():
                create_gnrs_submit_script(
                    ...
                    genarris_slurm_info=genarris_slurm_info,
...
def run_genarris_jobs(
    ...
    if genarris_slurm_config == {}:
        ...
        logger.info("Using default SLURM parameters for Genarris")
    else:
        logger.info(f"SLURM configuration for Genarris: {genarris_slurm_config}")

    executor = submitit.AutoExecutor(folder=output_dir.parent / "slurm")
    executor.update_parameters(
        slurm_job_name=genarris_slurm_config.get("job-name", "genarris"),
        nodes=genarris_slurm_config.get("nodes", 1),
        tasks_per_node=genarris_slurm_config.get("ntasks-per-node", 1),
        timeout_min=genarris_slurm_config.get("time", 7200),
