...
Test (C, third try): set
``torch._dynamo.config.assume_static_by_default = False`` BEFORE the
predictor builds. Tells dynamo to treat tensor dims as symbolic from
the very first compile, instead of specializing on the first call's
shape and recompiling on the second.
"""
...
# IMPORTANT: set BEFORE any torch.compile / model construction.
...
def main():
    ...
    print("config: torch._dynamo.config.assume_static_by_default = False")
